...
```bash
# Claude가 한 줄로 검색을 위임하고, 답만 돌려받습니다:
$ qvts -p ./app --json "loadConfig 선언 위치랑 그걸 부르는 곳 전부"
{"answer":"config-loader.mjs:40\nagent-core.mjs:14,16\nvts-bridge.mjs:29,31", ...}
#  → 약 20토큰. 검색은 로컬 모델이 했고, Claude는 원본 결과를 본 적 없습니다.
```
...
node dashboard.mjs                                          # 동작 구경 → http://127.0.0.1:7878
```

**Claude가 알아서 위임하게:** Claude Code 플러그인으로 설치하거나(또는
[`claude-routing.md`](claude-routing.md) 내용을 `CLAUDE.md`에 붙여넣기). 그러면 Claude가 검색·큰 읽기를
스스로 넘깁니다.

## 무엇을 할 수 있나요

**코드 찾기** (로컬 모델이 검색 → `file:line`을 받음):
- `qvts "<질문>"` — 심볼·호출처·파일·텍스트 찾기. `def_search`는 "X 어디 선언됐어"에 맞는 패턴을 언어별로
  자동으로 만들어 줍니다.
- `qvts --batch '["q1","q2",…]'` — 한 세션에서 여러 검색을 한 번에.

**코드 읽기** (로컬 모델이 읽고 → 요약을 받음):
- `qvts digest <파일> --focus "..."` — 큰 파일을 Claude에 안 넣고 요약.
- `qvts digest-dir <폴더>` — 폴더 전체를 요약 하나로.
- `qvts triage-diff` — git diff를 "중요한 건 이거, 이 파일들 열어봐"로 정리.

**동작 보기:**
- `node dashboard.mjs` — 모델이 무슨 일을 하는지 프로젝트·작업종류별로 묶어 실시간으로 보여주고, 아낀
  토큰도 표시하는 로컬 페이지.

모든 작업은 캐시되고(같은 질문은 즉시·무료) 절약 내역이 기록됩니다(`qvts --savings`).
