...
4. 베이스 `qwen2.5-coder:14b-instruct-q4_K_M` pull → `Modelfile.qwen25-14b`로
   `qwen25-14b-vts` 빌드 (`num_ctx 32768`, `num_gpu 999`)
...
`Modelfile.qwen25-14b` 의 `num_ctx` 조정 후 재빌드:
`ollama create qwen25-14b-vts -f Modelfile.qwen25-14b`
...
베이스 모델 교체 시 `Modelfile.qwen25-14b` 의 `FROM` 줄 변경.
